...
public sealed class GVxClient : IDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new GVxApiException("Unable to call the local gVx middleware.", innerException: ex);
        }
        ...
        using (response)
        {
            ...
            {
                throw new GVxApiException("Unable to parse the gVx middleware response.", response.StatusCode, body, ex);
            }
        }
    }
    ...
    internal static JsonElement GetRequiredProperty(JsonElement response, string propertyName)
    {
        ...
        {
            throw new GVxApiException($"gVx response does not contain '{propertyName}'.", responseBody: response.GetRawText());
        }
    ...
    private static GVxApiException BusinessException(JsonElement response, int code, string messageProperty)
    {
        var message = response.TryGetProperty(messageProperty, out var value) ? value.ToString() : "Unknown API error";
        return new GVxApiException($"gVx API error: code={code}, message={message}", responseBody: response.GetRawText());
    }
