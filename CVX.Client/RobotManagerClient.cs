...
public sealed class RobotManagerClient : IDisposable
{
    ...
    public RobotManagerClient(Uri? baseAddress = null, TimeSpan? timeout = null)
        : this(new HttpClient(), baseAddress, ownsHttpClient: true, timeout)
    {
    }
    ...
    internal async Task ConfigureConnectionAsync(
        WechatRobotOptions options, string callbackUrl, CancellationToken cancellationToken)
    {
        // Only patch explicitly supplied persistent settings; preserve Manager defaults.
        var body = new Dictionary<string, object> { ["httpCallbackUrl"] = callbackUrl };
        if (options.UpdateServerUrl is not null) body["updateServerUrl"] = options.UpdateServerUrl;
